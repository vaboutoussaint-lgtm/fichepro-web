...
def get_license_key():
    """Récupère la clé de licence depuis le cookie → Supabase"""
    sid = get_session_id()
    if not sid:
        return ""
    # Vérifier d'abord en mémoire Flask (plus rapide)
    key = session.get("license_key", "")
    if key:
        return key
    # Sinon lire depuis Supabase
...
def get_engine():
    ...
    Stratégie : cookie → session → moteur en mémoire → Supabase
    """
    license_key = get_license_key()
    
    # Si clé trouvée et moteur en mémoire avec données
    if license_key and license_key in _engines and _engines[license_key].base_parcelles:
        return _engines[license_key]
    
    # Si clé trouvée mais moteur absent ou vide → charger depuis Supabase
    if license_key:
        _engines[license_key] = _init_engine(license_key)
        if _engines[license_key].base_parcelles:
            return _engines[license_key]
    
    # Fallback : chercher n'importe quel moteur en mémoire avec des données
    # (cas où le cookie n'est pas transmis mais le moteur existe)
...
def check_licensed():
    ...
    # Si license.json absent, vérifier via la session uniquement
    if not info.get("key"):
        return True, key  # Session valide = accès autorisé
    return is_licensed(), key
...
@app.route("/app")
def app_main():
    licensed, key = check_licensed()
    if not licensed or not key:
        return redirect("/")
