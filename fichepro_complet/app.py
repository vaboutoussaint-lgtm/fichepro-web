...
def get_engine() -> FicheEngine:
    """Charge ou recharge le moteur depuis la config persistante sur disque."""
    global _engine
    cfg = get_config()
    ...
    if _engine is None or _engine.wb_path != wb_path:
        ...
        # Restaurer le calendrier depuis la config
...
def api_import_ra():
    ...
    engine = get_engine()
    if engine is None:
        return jsonify({"erreur": "Configurez d'abord votre fichier FichePro Manager (.xlsm)."})
    tmp = DATA_DIR / ("ra_" + f.filename)
    f.save(str(tmp))
    # Import complet : charge producteurs + calcule STK_GT (80%) + STK_PT (20%)
    result = engine.importer_registre_ra(str(tmp))
    tmp.unlink(missing_ok=True)
    if result.get("succes"):
        # Recharger en mémoire après import
        engine.init_dict_lignes()
        engine.precharger_memoire()
