...
def get_license_key():
    """Récupère la clé de licence depuis la session Flask.
    La session persiste tant que le navigateur est ouvert.
    """
    return session.get("license_key", "DEFAULT").upper()

def get_engine():
    """Retourne le moteur isolé pour la licence active"""
    license_key = get_license_key()
    
    # Si pas de clé dans la session, chercher le premier moteur disponible
    if license_key == "DEFAULT":
        # Chercher un moteur existant avec des producteurs
        for key, eng in _engines.items():
            if eng and eng.base_parcelles:
                return eng
        # Sinon charger depuis Supabase sans filtre licence
        return FicheEngine("")
    
    if license_key not in _engines or not _engines[license_key].base_parcelles:
        _engines[license_key] = _init_engine(license_key)
    return _engines[license_key]

def _init_engine(license_key):
    """
    Initialise un moteur depuis Supabase.
    Recalcule les stocks en déduisant toutes les livraisons des fiches déjà générées.
    """
    engine = FicheEngine("")
    ...
    if prods:
        ...
        engine.dict_lignes = {}
        # Index par parcelle pour accès rapide
        parc_index = {}
        ...
        # Recalculer les stocks depuis les fiches Supabase
        # Remettre les stocks à l'estimation initiale
        for bp in engine.base_parcelles:
            est = bp["estim"]
            bp["stk_gt"] = int(est * 0.8)
            bp["stk_pt"] = int(est * 0.2)
            bp["liv_gt"] = 0
            bp["liv_pt"] = 0

        # Déduire les volumes de chaque fiche déjà générée
        try:
            fiches = charger_fiches(license_key)
            for fiche in fiches:
                lignes_json = fiche.get("lignes", "[]")
                try:
                    lignes = json.loads(lignes_json) if isinstance(lignes_json, str) else lignes_json
                except:
                    lignes = []
                periode = fiche.get("periode", "")
                est_gt = "grande traite" in periode.lower() if periode else True
                for ligne in lignes:
                    parc = ligne.get("parc", "")
                    poids = int(ligne.get("poids", 0))
                    if parc in parc_index:
                        idx = parc_index[parc]
                        bp = engine.base_parcelles[idx]
                        if est_gt:
                            bp["liv_gt"] += poids
            ...
            print(f"Erreur recalcul stocks: {e}")
...
def api_verify_licence():
    ...
    if result.get("valid"):
        lk = key.upper()
        # Stocker dans la session Flask (persiste tant que le navigateur est ouvert)
        session["license_key"] = lk
        session.permanent = True
        # Précharger les données depuis Supabase
        if lk not in _engines:
...
def api_import_ra():
    ...
    if result.get("succes"):
        # Mettre à jour le moteur en mémoire directement
