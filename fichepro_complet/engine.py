...
class FicheEngine:
    ...
    def importer_registre_ra(self, ra_path: str) -> dict:
        """
        Lit le registre RA et met à jour la Base Parcelles du fichier Excel.
        Structure fixe : onglets '1. l'exploitation agricole',
        '2. Produit agricole certifié', '3. Unité agricole'
        """
        try:
            wb_ra = load_workbook(ra_path, data_only=True)
            # Trouver les onglets (noms partiels)
            ...
            ws2 = find_sheet(wb_ra, "agricole certifi")
            ws3 = find_sheet(wb_ra, "unit")

            if not ws1:
                return {"erreur": "Onglet 'exploitation agricole' introuvable dans le registre RA."}

            # Lire onglet 1 : ID, Village, Région inspection, Prénom, Nom, Superficie
            prod_data = {}
            for row in ws1.iter_rows(min_row=3, values_only=True):
                if not row or row[0] in (None, ""):
                    continue
                iid     = str(row[0]).strip()
                village = str(row[2]).strip() if row[2] else ""
                region  = str(row[4]).strip() if row[4] else ""   # col 5 = index 4
                prenom  = str(row[9]).strip()  if row[9]  else ""  # col 10 = index 9
                nom_f   = str(row[10]).strip() if row[10] else ""  # col 11 = index 10
                nom_complet = f"{prenom} {nom_f}".strip()
                prod_data[iid] = {
                    "code"   : iid,
                    "nom"    : nom_complet,
                    "sect"   : region,
            ...
            # Lire onglet 2 : Estimation récolte
            if ws2:
                for row in ws2.iter_rows(min_row=3, values_only=True):
                    ...
                    estim = 0
                    try:
                        estim = int(float(str(row[4]))) if row[4] not in (None, "") else 0
                    except:
                        pass
                    if iid in prod_data:
                        prod_data[iid]["estim"] = estim

            # Lire onglet 3 : Parcelle ID + GPS
            if ws3:
                for row in ws3.iter_rows(min_row=3, values_only=True):
                    if not row or row[0] in (None, ""):
                        continue
                    iid    = str(row[0]).strip()
                    parc_id = str(row[1]).strip() if row[1] else iid + "-P1"
                    try:
                    ...
                    try:
                        ...
                        lon = None
                    if iid in prod_data:
                        if not prod_data[iid]["parc"]:
                            prod_data[iid]["parc"] = parc_id
                        if lat is not None:
                            prod_data[iid]["lat"] = lat
                        if lon is not None:
                            prod_data[iid]["lon"] = lon

            if not prod_data:
                return {"erreur": "Aucun producteur trouvé dans le registre RA."}

            # Mettre à jour la Base Parcelles dans le fichier Excel
            wb_fp = load_workbook(self.wb_path, keep_vba=True)
            ws_bp = wb_fp["Base Parcelles"]

            # Lire codes existants
            codes_existants = {}
            for row in ws_bp.iter_rows(min_row=2):
                if row[COL_BP_CODE].value:
                    codes_existants[str(row[COL_BP_CODE].value)] = row[0].row
            ...
            for iid, p in prod_data.items():
                if p["estim"] <= 0:
                    continue
                stk_gt = int(p["estim"] * PCT_GT)
                stk_pt = int(p["estim"] * PCT_PT)
                if iid in codes_existants:
                    # Mettre à jour l'estimation uniquement
                    r = codes_existants[iid]
                    ws_bp.cell(r, COL_BP_ESTIM + 1, p["estim"])
                    ws_bp.cell(r, COL_BP_STK_GT + 1, stk_gt)
                    ws_bp.cell(r, COL_BP_STK_PT + 1, stk_pt)
                    mis_a_jour += 1
                else:
                    # Ajouter une nouvelle ligne
                    lr += 1
                    ws_bp.cell(lr, COL_BP_CODE + 1,   p["code"])
                    ws_bp.cell(lr, COL_BP_PARC + 1,   p["parc"] or p["code"] + "-P1")
                    ws_bp.cell(lr, COL_BP_NOM + 1,    p["nom"])
                    ws_bp.cell(lr, COL_BP_SECT + 1,   p["sect"])
                    ws_bp.cell(lr, COL_BP_ESTIM + 1,  p["estim"])
                    ws_bp.cell(lr, COL_BP_STK_GT + 1, stk_gt)
                    ws_bp.cell(lr, COL_BP_STK_PT + 1, stk_pt)
                    ws_bp.cell(lr, COL_BP_LIV_GT + 1, 0)
                    ws_bp.cell(lr, COL_BP_LIV_PT + 1, 0)
                    if p["lon"] is not None:
                        ws_bp.cell(lr, COL_BP_LON + 1, p["lon"])
                    if p["lat"] is not None:
                        ws_bp.cell(lr, COL_BP_LAT + 1, p["lat"])
                    ajoutes += 1

            wb_fp.save(self.wb_path)
            self._log(f"Import RA : {ajoutes} ajoutés, {mis_a_jour} mis à jour")
            # Recharger en mémoire
            self.init_dict_lignes()
            return {
                "succes"    : True,
                "ajoutes"   : ajoutes,
