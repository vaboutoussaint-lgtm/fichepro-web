...
class FicheEngine:
    ...
    # ============================================================
    def _detecter_format(self, wb) -> str:
        """Détecte si c'est le registre RA officiel ou le modèle client simplifié"""
        # Modèle client : feuille "Registre Producteurs" ou entête "Code Producteur"
        for name in wb.sheetnames:
            if "registre" in name.lower() or "producteur" in name.lower():
                return "CLIENT"
        # RA officiel : onglets avec "exploitation" et "certifi"
        for name in wb.sheetnames:
            if "exploitation" in name.lower():
                return "RA_OFFICIEL"
        return "CLIENT"

    def _importer_modele_client(self, wb) -> dict:
        """Import depuis le modèle Excel simplifié pour les clients"""
        # Trouver la bonne feuille
        ws = None
        for name in wb.sheetnames:
            if "registre" in name.lower() or "producteur" in name.lower() or name == wb.sheetnames[0]:
                ws = wb[name]
                break
        if not ws:
            return {"erreur": "Feuille introuvable dans le fichier."}

        prod_data = {}
        parcelles = []

        for row in ws.iter_rows(min_row=5, values_only=True):  # Données à partir ligne 5
            if not row or row[0] in (None, ""):
                continue
            # Ignorer les lignes d'exemple ou entêtes
            val0 = str(row[0]).strip()
            if any(kw in val0.lower() for kw in ["code", "ex:", "exemple", "producteur"]):
                continue

            code_prod = val0
            code_parc = str(row[1]).strip() if row[1] else f"{code_prod}-P1"
            nom       = str(row[2]).strip() if row[2] else code_prod
            section   = str(row[3]).strip() if row[3] else ""
            try:
                superficie = float(row[4]) if row[4] not in (None, "") else 0.0
            except:
                superficie = 0.0
            try:
                rendement_ha = float(row[5]) if row[5] not in (None, "") else 0.0
            except:
                rendement_ha = 0.0
            try:
                lat = float(row[6]) if row[6] not in (None, "") else None
            except:
                lat = None
            try:
                lon = float(row[7]) if row[7] not in (None, "") else None
            except:
                lon = None

            if superficie <= 0 or rendement_ha <= 0:
                continue

            estim = int(rendement_ha * superficie)
            if estim <= 0:
                continue

            if code_prod not in prod_data:
                prod_data[code_prod] = {"nom": nom, "sect": section}
        ...
        if not parcelles:
            return {"erreur": "Aucun producteur valide trouvé. Vérifiez que les colonnes Superficie et Rendement sont remplies."}
        ...
        for pa in parcelles:
            ...
            if pid in parc_existantes:
                self.base_parcelles[parc_existantes[pid]].update(pa)
                mis_a_jour += 1
            else:
                idx = len(self.base_parcelles)
                self.base_parcelles.append(pa)
                self.dict_lignes[pa["code"]] = idx
                parc_existantes[pid] = idx
                ajoutes += 1

        self._log(f"Import modèle client : {ajoutes} ajoutés, {mis_a_jour} mis à jour ({len(prod_data)} producteurs, {len(parcelles)} parcelles)")
        return {"succes": True, "ajoutes": ajoutes, "mis_a_jour": mis_a_jour,
                "total": len(parcelles), "nb_prod": len(prod_data)}
    ...
    def importer_registre_ra(self, ra_path: str) -> dict:
        ...
        try:
            ...
            # Détecter le format et router vers le bon parseur
            format_detecte = self._detecter_format(wb_ra)
            if format_detecte == "CLIENT":
                return self._importer_modele_client(wb_ra)
