...
class FicheEngine:
    ...
    # ============================================================
    def _charger_workbook_universel(self, path: str):
        """Charge un fichier Excel dans n'importe quel format : xlsx, xls, xlsm, xlsb"""
        import subprocess, os
        path = str(path)
        ext = os.path.splitext(path)[1].lower()
        
        # Essayer openpyxl d'abord (xlsx, xlsm, xltx)
        try:
            return load_workbook(path, data_only=True)
        except:
            pass
        
        # Si échec, convertir avec xlrd (xls ancien format)
        try:
            import xlrd
            wb_xls = xlrd.open_workbook(path)
            # Convertir en openpyxl
            ...
            for sheet_name in wb_xls.sheet_names():
                ...
                for row in range(ws_xls.nrows):
                    for col in range(ws_xls.ncols):
                        ...
                        ws_new.cell(row+1, col+1, cell.value)
            return wb_new
        except:
            pass
    ...
    def importer_registre_ra(self, ra_path: str) -> dict:
        ...
        """
        try:
            wb_ra = self._charger_workbook_universel(ra_path)
            if wb_ra is None:
                return {"erreur": "Format de fichier non supporté. Essayez de sauvegarder en .xlsx depuis Excel."}
            ...
            # ── Mise à jour Base Parcelles dans le fichier Excel FichePro ──
            wb_fp = load_workbook(self.wb_path, keep_vba=True)
            ws_bp = wb_fp["Base Parcelles"]

            # Lire parcelles existantes (clé = ID parcelle)
            parc_existantes = {}
            for row in ws_bp.iter_rows(min_row=2):
                val = row[COL_BP_PARC].value
                if val:
                    parc_existantes[str(val)] = row[0].row
            ...
            for iid, p in prod_data.items():
                ...
                for pa in p["parcelles"]:
                    ...
                    if pid in parc_existantes:
                        # Mise à jour estimation et stocks
                        r = parc_existantes[pid]
                        ws_bp.cell(r, COL_BP_ESTIM  + 1, pa["estim"])
                        ws_bp.cell(r, COL_BP_STK_GT + 1, pa["stk_gt"])
                        ws_bp.cell(r, COL_BP_STK_PT + 1, pa["stk_pt"])
                        if pa["lat"] is not None:
                            ws_bp.cell(r, COL_BP_LAT + 1, pa["lat"])
                        if pa["lon"] is not None:
                            ws_bp.cell(r, COL_BP_LON + 1, pa["lon"])
                        mis_a_jour += 1
                    else:
                        # Nouvelle ligne
                        lr += 1
                        ws_bp.cell(lr, COL_BP_CODE  + 1, iid)
                        ws_bp.cell(lr, COL_BP_PARC  + 1, pid)
                        ws_bp.cell(lr, COL_BP_NOM   + 1, p["nom"])
                        ws_bp.cell(lr, COL_BP_SECT  + 1, p["sect"])
                        ws_bp.cell(lr, COL_BP_ESTIM + 1, pa["estim"])
                        ws_bp.cell(lr, COL_BP_STK_GT + 1, pa["stk_gt"])
                        ws_bp.cell(lr, COL_BP_STK_PT + 1, pa["stk_pt"])
                        ws_bp.cell(lr, COL_BP_LIV_GT + 1, 0)
                        ws_bp.cell(lr, COL_BP_LIV_PT + 1, 0)
                        if pa["lat"] is not None:
                            ws_bp.cell(lr, COL_BP_LAT + 1, pa["lat"])
                        if pa["lon"] is not None:
                            ws_bp.cell(lr, COL_BP_LON + 1, pa["lon"])
                        parc_existantes[pid] = lr
                        ajoutes += 1

            wb_fp.save(self.wb_path)
            self._log(f"Import RA : {ajoutes} parcelles ajoutées, {mis_a_jour} mises à jour "
                      f"({len(prod_data)} producteurs, {total_parc} parcelles totales)")
            # Recharger en mémoire
            self.init_dict_lignes()
            self.precharger_memoire()
            return {
                "succes"    : True,
                "ajoutes"   : ajoutes,
