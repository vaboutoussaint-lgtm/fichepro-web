...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if record.get("expiry_date"):
            exp = datetime.strptime(record["expiry_date"], "%Y-%m-%d").date()
            if exp < date.today():
...
def login():
    ...
    if record.get("expiry_date"):
        exp = datetime.strptime(record["expiry_date"], "%Y-%m-%d").date()
        if exp < date.today():
            return jsonify({"ok": False, "error": "Licence expirée le " + record["expiry_date"]}), 403

    session["license_key"] = key
    # Vérifier si le client a déjà des producteurs
    prods = supabase.table("producers").select("id", count="exact").eq("license_key", key).execute()
    has_producers = (prods.count or 0) > 0
...
def me():
    key = get_license()
    prods = supabase.table("producers").select("id", count="exact").eq("license_key", key).execute()
    fiches = supabase.table("fiches").select("id", count="exact").eq("license_key", key).execute()
    return jsonify({
        "client_name": request.license.get("client_name"),
        "expiry_date": request.license.get("expiry_date"),
        "has_producers": (prods.count or 0) > 0,
        "producers_count": prods.count or 0,
        "fiches_count": fiches.count or 0
    })
...
def upload_excel():
    ...
    try:
        ...
        for name in wb.sheetnames:
            if any(kw in name.lower() for kw in ["prod", "registre", "agricult", "ra"]):
...
def list_producers():
    ...
    for p in data:
        fc = supabase.table("fiches").select("id", count="exact").eq("license_key", key).eq("producer_id", p["id"]).execute()
        p["fiches_count"] = fc.count or 0
    return jsonify(data)
...
def create_fiche():
    ...
    today = datetime.now()
    count_r = supabase.table("fiches").select("id", count="exact").eq("license_key", key).execute()
    num = f"FP-{today.strftime('%Y%m')}-{(count_r.count or 0) + 1:04d}"
