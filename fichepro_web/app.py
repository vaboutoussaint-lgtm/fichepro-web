    ...
    try:
        ...
        if target_sheet:
            if is_base_parcelles:
                # Structure réelle "Base Parcelles" (0-based) :
                # CODE=0, PARC=1, NOM=2, SECT=3, SURFACE=4, ESTIM=5,
                # max_par_parcelle=6, max_livrable=7, seuil_max_total=8
                col_map = {
...
def export_excel_fiche(fiche_id):
    ...
    # ───────────────────────────────────────────────────────────────────────────
    # Helpers (opèrent sur la feuille passée en argument)
    # ───────────────────────────────────────────────────────────────────────────
    def hdr(ws_, r, c, val, bg=C_GREEN, fg=C_WHITE, sz=9, wrap=False):
        cl = ws_.cell(row=r, column=c, value=val)
        cl.font = Font(bold=True, color=fg, size=sz)
    ...
    def dat(ws_, r, c, val="", align="center", sz=10, bg=None, bold=False):
        cl = ws_.cell(row=r, column=c, value=val)
        cl.font = Font(bold=bold, color="000000", size=sz)
        if bg:
            cl.fill = PatternFill("solid", fgColor=bg)
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # ONGLET 1 — Base Parcelles  (même structure que le fichier VBA)
    # ═══════════════════════════════════════════════════════════════════════════
    ws1 = wb.active
    ws1.title = "Base Parcelles"

    bp_hdrs = [
        "code producteur", "code_parcelle", "nom_prenom", "section",
        "superficie_ha", "estimation_ha", "max_par_parcelle",
        "max_livrable", "seuil_max_total", "",
        "StockDisponibleGT", "StockDisponiblePT",
        "DejaLivreGT", "DejaLivrePT",
        "Longitude", "Latitude"
    ]
    for ci, h in enumerate(bp_hdrs, start=1):
        if h:
            bg = C_GREEN if ci in (11, 12, 13, 14) else "4F81BD"
            hdr(ws1, 1, ci, h, bg=bg, sz=9)
    ws1.row_dimensions[1].height = 20

    for idx, p in enumerate(producteurs):
        r = idx + 2
        estim = float(p.get("estim") or 0) or float(p.get("surface_ha") or 1) * 500
        surf  = float(p.get("surface_ha") or 0)
        poids_p = float(p.get("poids", 0))
        dat(ws1, r, 1,  p.get("code",""),    align="center")
        dat(ws1, r, 2,  p.get("code",""),    align="center")
        dat(ws1, r, 3,  p.get("nom",""),     align="left")
        dat(ws1, r, 4,  p.get("section",""), align="center")
        dat(ws1, r, 5,  round(surf, 5),      align="center")
        dat(ws1, r, 6,  round(estim, 0),     align="center")
        dat(ws1, r, 7,  round(estim, 0),     align="center")
        dat(ws1, r, 8,  poids_p,             align="center")   # max_livrable = poids alloué
        dat(ws1, r, 9,  poids_p,             align="center")   # seuil_max_total
        dat(ws1, r, 10, "")
        dat(ws1, r, 11, "", align="center")
        dat(ws1, r, 12, "", align="center")
        dat(ws1, r, 13, "", align="center")
        dat(ws1, r, 14, "", align="center")
        dat(ws1, r, 15, "", align="center")
        dat(ws1, r, 16, "", align="center")
        ws1.row_dimensions[r].height = 16

    for ci, w in enumerate([14,14,26,12,12,14,14,14,14,4,14,14,12,12,12,12], start=1):
        ws1.column_dimensions[get_column_letter(ci)].width = w

    # ═══════════════════════════════════════════════════════════════════════════
    # ONGLET 2 — Suivi Livraisons  (même structure que le fichier VBA)
    # Colonnes : Code Producteur | Poids (kg) | Date | ID Fiche |
    #            Code Parcelle | Section | Période | Prix/kg | Montant
    # ═══════════════════════════════════════════════════════════════════════════
    ws2 = wb.create_sheet("Suivi Livraisons")

    sl_hdrs = ["Code Producteur", "Poids (kg)", "Date", "ID Fiche",
               "Code Parcelle", "Section", "Période", "Prix/kg", "Montant"]
    for ci, h in enumerate(sl_hdrs, start=1):
        hdr(ws2, 1, ci, h, sz=9)
    ws2.row_dimensions[1].height = 20

    cal_label = fiche.get("calendrier", "")
    fiche_id  = str(fiche.get("id", ""))[:8]
    date_f    = fiche.get("date_fiche", "")
    r = 2
    if producteurs:
        for idx, p in enumerate(producteurs):
            poids_p = float(p.get("poids", 0))
            mont_p  = round(poids_p * prix, 0)
            bg_row  = C_WHITE if idx % 2 == 0 else "F0FAF2"
            dat(ws2, r, 1, p.get("code",""),    align="center", bg=bg_row)
            dat(ws2, r, 2, poids_p,             align="center", bg=bg_row)
            dat(ws2, r, 3, date_f,              align="center", bg=bg_row)
            dat(ws2, r, 4, fiche_id,            align="center", bg=bg_row)
            dat(ws2, r, 5, p.get("code",""),    align="center", bg=bg_row)
            dat(ws2, r, 6, p.get("section",""), align="center", bg=bg_row)
            dat(ws2, r, 7, cal_label,           align="center", bg=bg_row)
            dat(ws2, r, 8, prix,                align="center", bg=bg_row)
            dat(ws2, r, 9, mont_p,              align="center", bg=bg_row)
            ws2.row_dimensions[r].height = 16
            r += 1
    else:
        dat(ws2, r, 1, "—"); dat(ws2, r, 2, float(fiche.get("poids_brut",0)))
        dat(ws2, r, 3, date_f); dat(ws2, r, 4, fiche_id)
        dat(ws2, r, 5, "—"); dat(ws2, r, 6, "—"); dat(ws2, r, 7, "—")
        dat(ws2, r, 8, prix); dat(ws2, r, 9, float(fiche.get("montant_total",0)))
        r += 1

    # Ligne TOTAL
    total_p = sum(float(p.get("poids",0)) for p in producteurs) if producteurs else float(fiche.get("poids_brut",0))
    dat(ws2, r, 1, "TOTAL", bold=True, bg=C_GRAY)
    dat(ws2, r, 2, total_p,             bold=True, bg=C_GRAY, align="center")
    dat(ws2, r, 3, "", bg=C_GRAY); dat(ws2, r, 4, "", bg=C_GRAY)
    dat(ws2, r, 5, "", bg=C_GRAY); dat(ws2, r, 6, "", bg=C_GRAY)
    dat(ws2, r, 7, "", bg=C_GRAY); dat(ws2, r, 8, prix, bg=C_GRAY, align="center")
    dat(ws2, r, 9, round(total_p * prix, 0), bold=True, bg=C_GRAY, align="center")
    ws2.row_dimensions[r].height = 18

    for ci, w in enumerate([16, 12, 12, 14, 16, 12, 24, 10, 14], start=1):
...
def auto_fiches():
    ...
    def capacite(p):
        # estimation_ha importée depuis Base Parcelles (col index 5)
        # Fallback : surface_ha * 500 si estim absent ou nul
        e = float(p.get("estim") or 0)
        return e if e > 0 else float(p.get("surface_ha") or 1) * 500
